...
import com.vladimirbaklan.products.{DefaultProductsService, JooqProductsDao}
...
object Application extends App {
  ...
  val productsService = new DefaultProductsService(new JooqProductsDao(DatabaseWiring.Jooq))
  ...
  val handler = new EventsHandler(new ProductsEventProcessor(productsService, new EventProducer(producerProperties)), consumerProperties)
  handler.start
}
